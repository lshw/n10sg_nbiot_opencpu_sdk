 ...
 *  \更多信息参阅《N10SG_AT_Command_Interface_Specification.pdf》中AT+CMSYSCTRL指令说明；
 *  \
 */
 ...
 *  \brief band信息写入
   *  
   *  \param [in] writeband 结构体成员中0表示关闭对应的band，1表示开启。全0为恢复默认值，全1在N10SG中不支持。
   *  \return 0 成功 1 失败
   *  
 */
 ...
 *
   N10SG目前支持基于onenet平台的差分固件升级，平台操作方法请参考OneNET平台发布的FOTA用户使用说明中"提供差分能力的升级系统界面"，部分相关文档请在OneNET平台上获取。
   FOTA操作方法请参见FOTA相关手册
 *@{
 */
 ...
 *  \brief 
 *  
 *  \param [in] chan ADC通道号，支持HAL_ADC_CHANNEL_0，HAL_ADC_CHANNEL_1，HAL_ADC_CHANNEL_2
 *  \return 电压值
 *
 */
 ...
 *  \brief spi初始化
 *
 *  \param [in] bus spi总线,0/1
 *  \param [in] config spi配置
 *  \return true or false
 *
 *  \details Details
 */
bool opencpu_spi_init_ex(uint8_t bus, hal_spi_master_config_t* config);
 ...
 *  \details Details
 */
bool openpcu_spi_deinit_ex(uint8_t bus);

/**
 *  \brief spi读写
 *
 *  \param [in] bus spi总线,0/1
 *  \param [in] send_data 发送数据
 *  \param [in] send_len 发送数据长度
 *  \param [in] recv_data 接收数据
 *  \param [in] recv_len 接收数据长度
 *  \return spi状态码,参见hal_spi_master_status_t定义
 *
 *  \details Details
 */
int opencpu_spi_write_read_ex(uint8_t bus, char* send_data, uint32_t send_len, char* recv_data, uint32_t recv_len);
 ...
 *  \details Details
 */
int opencpu_spi_write_ex(uint8_t bus, char* data, uint32_t len);
 ...
 *  \details Details
 */
int opencpu_spi_read_ex(uint8_t bus, char* data, uint32_t len);
  /** @} */
     /**
 ...
 *  \brief i2c初始化
 *
 *  \param [in] bus i2c总线,0/1
 *  \param [in] i2c_config i2c配置
 *  \return true or false
 *
 *  \details Details
 */
bool opencpu_i2c_init_ex(uint8_t bus, hal_i2c_config_t* i2c_config);
 ...
 *  \details Details
 */
bool opencpu_i2c_deinit_ex(uint8_t bus);
 ...
 *  \param [in] data 待发送数据
 *  \param [in] len 数据长度
 *  \return i2c状态码,参见hal_i2c_status_t定义
 *
 *  \details Details
 */
int opencpu_i2c_write_ex(uint8_t bus, uint8_t slave_addr, uint8_t *data, uint16_t len);
 ...
 *  \details Details
 */
int opencpu_i2c_read_ex(uint8_t bus, uint8_t slave_addr, uint8_t *data, uint16_t len);
 ...
 *  \details Details
 */
int opencpu_i2c_write_read_ex(uint8_t bus, uint8_t slave_addr, uint8_t reg, uint8_t *data, uint16_t len);
  /** @} */

   /**

 * @defgroup GPIO_FUNCTIONS
 * N10SG 模组只要没被复用成特殊功能的引脚，都可以当做普通GPIO来使用，模组引脚号、MT2625GPIO号、opencpu引脚功能名称三者的对应关系见附件Excel文件，相关结构体请参考hal_gpio.h，配置GPIO的接口函数如下所示。\n
  1.引脚初始化
  @code
 ...
 * N10SG使用FreeRTOS V8.2.0嵌入式操作系统，所有的接口和标准FREERTOS一致，遇到相关问题，开发者可登录https://www.freertos.org/搜索相关资料解决，也可联系我们解决。相关结构体、参数等可以参考os目录下头文件，此处给出部分主要功能函数示例
 *  \n示例1：任务创建
  *<pre>
